...
public class OldColorToCoursesEnumerator implements Iterator<Pair<Integer, List<Integer>>> {
	...
	public OldColorToCoursesEnumerator (Map<Integer, List<Integer>> colorsToSets, Integer numberOfClassrooms) {
	...
	public Pair<Integer, List<Integer>> next() {
		...
		if(returnedCourses.isEmpty() == false) {
			while(true) {
				if(returnedCourses.isEmpty() == true) {
					enumerateFurther = false;
					break;
				}
				
				Pair<Integer, Integer> top = returnedCourses.pop();
				int remaining = coursesWithSameColor.size() - top.getRight() - 1;
		 ...
		 */
		if((coursesWithSameColor.size() - _numberOfClassrooms) == endIdx) {
			enumerateFurther = false;
